...
RSpec.shared_examples "PUT #update" do |model, attrs, parent = nil|
  ...
  context "with valid params" do
    ...
    it "updates the requested #{model}" do
      ...
      put :update, params: {'id'=> obj.to_param, "#{model}"=> new_attributes}, session: valid_session
      obj.reload
    ...
    it "renders a JSON response with the #{model}" do
      ...
      put :update, params: { 'id'=> obj.to_param, "#{model}"=> valid_attributes }, session: valid_session
      expect(response).to have_http_status(:ok)
      expect(response.content_type).to eq('application/json')
    end
  end
  ...
  context "with invalid params" do
    it "renders a JSON response with errors for the #{model}" do
      ...
      put :update, params: { 'id'=> obj.to_param, "#{model}"=> invalid_attributes }, session: valid_session
      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.content_type).to eq('application/json')
    end
  end
